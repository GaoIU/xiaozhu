layui.use(['form', 'element', 'layer', 'laydate'], function() {
	...
	$('.create').on('click', function() {
		layer.open({
			...
			type: 2,
			area: ['100%', '100%'],
			content: '/model/gotoInfo',
			success: function(index, layero) {
				setTimeout(function() {
					layer.tips('点击此处返回模版列表', '.layui-layer-setwin .layui-layer-close', {
						tips: 3
					});
				}, 500);
			}
...
var queryList = new Vue({
	...
	methods: {
		find() {
			var URL = "/modelBuild?current=" + pageShow.current + "&size=" + pageShow.size + "&" + $('#searchForm').serialize();
		...
		edit(id) {
			...
			layer.open({
				...
				type: 2,
				area: ['100%', '100%'],
				content: '/modelBuild/gotoInfo?id=' + id,
				success: function(index, layero) {
					setTimeout(function() {
						layer.tips('点击此处返回模版明细列表', '.layui-layer-setwin .layui-layer-close', {
							tips: 3
						});
					}, 500);
				}
