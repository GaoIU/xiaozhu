layui.use(['form', 'element', 'layer', 'laydate'], function() {
	...
	$('.create').on('click', function() {
		layer.open({
			...
			type: 2,
			area: ['100%', '100%'],
			content: '/sysConfig/gotoInfo',
			success: function(index, layero) {
				setTimeout(function() {
					layer.tips('点击此处返回系统配置列表', '.layui-layer-setwin .layui-layer-close', {
						tips: 3
					});
				}, 500);
			}
...
var queryList = new Vue({
	...
	methods: {
		find() {
			var URL = "/sysConfig?current=" + pageShow.current + "&size=" + pageShow.size + "&" + $('#searchForm').serialize();
		...
		edit(id) {
			...
			layer.open({
				...
				type: 2,
				area: ['100%', '100%'],
				content: '/sysConfig/gotoInfo?id=' + id,
				success: function(index, layero) {
					setTimeout(function() {
						layer.tips('点击此处返回系统配置列表', '.layui-layer-setwin .layui-layer-close', {
							tips: 3
