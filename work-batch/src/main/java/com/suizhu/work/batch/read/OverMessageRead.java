...
@Component
@AllArgsConstructor
public class OverMessageRead implements ItemReader<Map<String, Object>> {
	...
	public Map<String, Object> read()
		...
		QueryWrapper<BuildProject> query = new QueryWrapper<>();
		query.eq("early", BuildProject.EARLY_OVERTIME_DOING).and(i -> i.eq("status", BuildProject.STATUS_DOING));
		query.select("id", "build_enginer_id", "name");
		List<BuildProject> list = buildProjectService.list(query);
		...
		if (CollUtil.isNotEmpty(list)) {
			...
			if (CollUtil.isNotEmpty(beIds)) {
				...
				umqw.eq("type", UserMessage.TYPE_OVERTIME).and(i -> i.in("doorway_id", doorwayIds))
				...
				if (count <= 0) {// 未发送推送
					if (CollUtil.isNotEmpty(beSet)) {
						...
						wrapper.eq("early", BuildProject.EARLY_OVERTIME_DOING)
								.and(i -> i.eq("status", BuildProject.STATUS_DOING));
						wrapper.and(i -> i.in("build_enginer_id", beSet)).select("id", "build_enginer_id", "name");
