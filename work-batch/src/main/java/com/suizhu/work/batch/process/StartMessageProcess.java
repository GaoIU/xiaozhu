...
@Component
@AllArgsConstructor
public class StartMessageProcess implements ItemProcessor<Map<String, Object>, List<UserMessage>> {
	...
	public List<UserMessage> process(Map<String, Object> item) throws Exception {
		if (CollUtil.isNotEmpty(item)) {
			...
			String jpushBodyStart = myContansConfig.getJpushBodyStart();
			String messageContentStart = myContansConfig.getMessageContentStart();
			...
			doorways.forEach(d -> {
				...
				for (BuildProject p : list) {
					if (StrUtil.equals(buildEnginer.getId(), p.getBuildEnginerId())) {
						if (count > 0) {
							sb.append("、");
						}
						sb.append(p.getName());
				...
				if (count > 0) {
					...
					if (CollUtil.isNotEmpty(userIds)) {
						...
						String message = String.format(jpushBodyStart, d.getName(), count);
						JpushUtil.jpush(jpushAppKey, jpushMasterSecret, jpushUrl, usernames, message);

						String body = String.format(messageContentStart, date, d.getName(), sb.toString());
						userIds.forEach(s -> {
							UserMessage userMessage = new UserMessage(name, body, UserMessage.TYPE_START, d.getId(), s);
