...
@Component
@AllArgsConstructor
public class EarlyMessageProcess implements ItemProcessor<Map<String, Object>, List<UserMessage>> {
	...
	public List<UserMessage> process(Map<String, Object> item) throws Exception {
		if (CollUtil.isNotEmpty(item)) {
			...
			String jpushBodyEarly = myContansConfig.getJpushBodyEarly();
			String messageContentEarly = myContansConfig.getMessageContentEarly();
			...
			doorways.forEach(d -> {
				...
				if (count > 0) {
					...
					if (CollUtil.isNotEmpty(userIds)) {
						...
						String message = String.format(jpushBodyEarly, d.getName(), count);
						JpushUtil.jpush(jpushAppKey, jpushMasterSecret, jpushUrl, usernames, message);

						String body = String.format(messageContentEarly, d.getName(), sb.toString());
						userIds.forEach(s -> {
							UserMessage userMessage = new UserMessage(name, body, UserMessage.TYPE_EARLY, d.getId(), s);
