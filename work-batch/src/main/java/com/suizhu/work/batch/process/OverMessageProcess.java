...
@Component
@AllArgsConstructor
public class OverMessageProcess implements ItemProcessor<Map<String, Object>, List<UserMessage>> {
	...
	public List<UserMessage> process(Map<String, Object> item) throws Exception {
		if (CollUtil.isNotEmpty(item)) {
			...
			String jpushBodyOvertime = myContansConfig.getJpushBodyOvertime();
			String messageContentOvertime = myContansConfig.getMessageContentOvertime();
			...
			doorways.forEach(d -> {
				...
				if (count > 0) {
					...
					if (CollUtil.isNotEmpty(userIds)) {
						...
						String message = String.format(jpushBodyOvertime, d.getName(), count);
						JpushUtil.jpush(jpushAppKey, jpushMasterSecret, jpushUrl, usernames, message);

						String body = String.format(messageContentOvertime, d.getName(), sb.toString());
						userIds.forEach(s -> {
							UserMessage userMessage = new UserMessage(name, body, UserMessage.TYPE_OVERTIME, d.getId(),
									s);
